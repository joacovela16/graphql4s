...
import modux.graphql.core.Parser._
import modux.graphql.defaults.{JsonRenderer, LinkImpl}
import modux.graphql.model.executor._
import modux.graphql.model.graphql._
import modux.graphql.model._
...
object GraphQL {
  ...
  private case class Context(fragments: Map[String, FragmentDef], accessor: Accessor, renderer: modux.graphql.model.Renderer)
  ...
  private def renderField(fieldName: String, obs: Observable[String], renderer: modux.graphql.model.Renderer): Observable[String] = {
    obs
      .isEmpty
      .flatMap { isEmpty =>
        if (isEmpty) {
  ...
  private def projector(value: Executor, expr: Iterable[Expr], context: Context): COMPUTE = {
    val renderer: modux.graphql.model.Renderer = context.renderer
    value match {
  ...
  private def introspection(binding: Binding): Try[Executor] = {
    ...
    querySystem match {
      ...
      case None => Failure(new RuntimeException("Query modux.graphql.model must be instantiate"))
    }
  }
